    ...
    def _run_swing(candles, balance, leverage, position_size, bt, params, step_delay=0):
        ...
        for i in range(50, len(closes)):
            ...
            if rsi_val < 30:
                sl = round(cur * (1 - SL_PCT), 4)
                tp = round(cur * (1 + TP_PCT), 4)
                position = {"type": "BUY", "entry": cur, "sl": sl, "tp": tp, "bar": i, "qty": qty, "margin": margin, "best": cur}
                bt["events"].append({
                    "ts": i, "type": "signal", "side": "BUY",
                    "price": round(cur, 4), "rsi": round(rsi_val, 1),
                    "sl": sl, "tp": tp
                ...
                sl = round(cur * (1 + SL_PCT), 4)
                tp = round(cur * (1 - TP_PCT), 4)
                position = {"type": "SELL", "entry": cur, "sl": sl, "tp": tp, "bar": i, "qty": qty, "margin": margin, "best": cur}
                bt["events"].append({
                    "ts": i, "type": "signal", "side": "SELL",
                    "price": round(cur, 4), "rsi": round(rsi_val, 1),
                    "sl": sl, "tp": tp
