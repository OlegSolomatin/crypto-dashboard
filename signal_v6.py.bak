#!/usr/bin/env python3
"""
СИГНАЛЬЩИК v6 — потоковые данные + TV-индикаторы + стакан
Главное отличие от v5: работает на потоковых данных (1 сек), учитывает:
  • SuperTrend + VWAP + ATR Stops + Volume Profile POC
  • Дисбаланс стакана (bid/ask глубина, стены)
  • SL/TP от стакана (НЕ фиксированные)
  • Уверенность сигнала (confidence 1-10)

Запуск: cron каждые 30 секунд (вместо 5 минут)
"""
...
# ═══ СТРАТЕГИЯ (параметры после анализа сделок 18.05.2026) ═══
# Проблема: 30.8% WR, SL 0.8% выбивало на шуме, ADX=25 пропускал слабый тренд
# Решение: SL шире (1.2%), TP 2:1, confidence≥5, ADX=20, RSI-фильтр жёстче
PARAMS = {
    "buy_rsi": 30,              # было 35 — ждём глубокой перепроданности
    "sell_rsi": 75,             # было 70 — ждём уверенного перекупа
    "sl_price_pct": 0.012,      # было 0.008 — 1.2% вместо 0.8% (шум TON ~0.5%)
    "tp_price_pct": 0.024,      # было 0.016 — 2.4% (сохраняем риск:вознаграждение 1:2)
    "timeout_minutes": 180,
    "min_confidence": 5,        # было 3 — фильтруем слабые сигналы
    "cooldown_minutes": 15,     # NEW — пауза после убытка
    "max_same_direction": 1,    # NEW — не более 1 сделки в одном направлении подряд
    "adx_threshold": 20,        # было 25 — ловим тренд раньше
    "rsi_confirm_bars": 2,      # NEW — RSI должен быть за порогом 2+ свечи подряд
}
...
def signal_v6(prices, high, low, volumes) -> Optional[Dict]:
    ...
    if cooldown > 0:
        try:
            ...
            last_closed = conn2.execute(
                "SELECT closed_ts, pnl FROM paper_positions WHERE closed_ts IS NOT NULL ORDER BY id DESC LIMIT 1"
            ).fetchone()
            conn2.close()
            if last_closed and last_closed[1] is not None and last_closed[1] < 0:
                last_dt = datetime.fromisoformat(last_closed[0])
                minutes_ago = (datetime.now(UTC_PLUS_3) - last_dt).total_seconds() / 60
                if minutes_ago < cooldown:
                    cooldown_active = True
                    book_reasons.append(f"Кулдаун {cooldown} мин после убытка (прошло {minutes_ago:.0f} мин)")
        except:
            pass
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS signals_v6 (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ts TEXT, signal TEXT, strength TEXT, confidence INTEGER,
        price REAL, sl REAL, tp REAL, rsi REAL, trend TEXT,
        tv_indicators TEXT, book_metrics TEXT
    )""")
    conn.execute("""INSERT INTO signals_v6 (ts, signal, strength, confidence, price, sl, tp, rsi, trend, tv_indicators, book_metrics)
        VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
...
if __name__ == "__main__":
    ...
    try:
        ...
        last_sig = last_check.execute(
            "SELECT ts FROM signals_v6 WHERE signal IS NOT NULL ORDER BY id DESC LIMIT 1"
        ).fetchone()
        last_check.close()
